...
\vspace{0.1cm}
\Indm
\Input{Root node $root$, Depth of $root$ $d$}
\Output{List of articulation points $L_A$, List of biconnected groups $L_G$}
\Indp
\vspace{0.1cm}
$visited$[$root$] $\gets$ TRUE\\
$depth$[$root$] $\gets d$\\
$low$[$root$] $\gets d$\\
$children \gets 0$\\
$articulation \gets$ FALSE\\
$S$.\push{root}\\
\ForEach{neighbour $n$ of $root$}{
  \uIf{\textbf{not} $visited$[$n$]}{
    $parent$[$n$] $\gets root$\\
    \ArticulationPoints{$n$, $d + 1$}\\
    $children \gets children + 1$\\
    \tcp{Build the biconnected component under current root}
    \If{$low$[$n$] $\ge depth$[$root$]}{
      $articulation \gets$ TRUE\\
      $G \gets \{root\}$\\
    ...
    $low$[$root$] $\gets$ \min{$low$[$root$], $low$[$n$]}\\
  }\ElseIf{$n \not= parent$[$root$]}{
    $low$[$root$] $\gets$ \min{$low$[$root$], $depth$[$n$]}
  }
}
\vspace{0.15cm}
\tcp{Append current node to list of articulation points}
\tcp{Distinguish if current node is root of DFS}
  \If{($parent$[$root$] == NULL \textbf{and} $children > 1$) \textbf{or} ($parent$[$root$] $\not=$ NULL \textbf{and} articulation)}{
  append $root$ to $L_A$\\
}
\end{algorithm}
\DecMargin{2em}
